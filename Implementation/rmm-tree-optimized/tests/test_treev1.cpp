...
class RMMTreeFixtureTest : public ::testing::Test{
    public:
        ...
        void SetUp(){
            ...
            t_v1 = new RMMTree_Kary_V1(v,sizeBlock,w,order);
            tBin = new RMMTree_Bin(v,sizeBlock,w);
            bps = new bp_support_sada<>(&(t_v1->bv));
            t_v1->buildingTree();
            tBin->buildingTree();
            GenerateARguments();
        }
...
TEST_F(RMMTreeFixtureTest, fwdSearch_general){
    for(int i=0;i<argsFindOpen.size();i++){
        EXPECT_EQ(t_v1->fwdSearch(argsFindOpen[i],argsDepth[i]),tBin->fwdSearch(argsFindOpen[i],argsDepth[i])) << "Resposta errada ao calcular um FwdSearch genérico, para i=" << argsFindOpen[i];
    } 
}

TEST_F(RMMTreeFixtureTest, fwdSearch_findClose){
    for(int i=0;i<argsFindClose.size();i++){
        EXPECT_EQ(t_v1->findClose(argsFindClose[i]),tBin->findClose(argsFindClose[i])) << "Resposta errada ao calcular o find_close de i=" << argsFindClose[i];
    }
}

TEST_F(RMMTreeFixtureTest, bwdSearch_general){ 
    for(int i=0;i<(int)argsFindOpen.size();i++){
       EXPECT_EQ(t_v1->bwdSearch(argsFindOpen[i],argsDepthBwd[i]),tBin->bwdSearch(argsFindOpen[i],argsDepthBwd[i])) << "Resposta errada ao calcular um BwdSearch genérico, para i=" << argsFindOpen[i];
    } 
}

TEST_F(RMMTreeFixtureTest, bwdSearch_findOpen){
    for(int i=0;i<argsFindOpen.size();i++){
        EXPECT_EQ(t_v1->findOpen(argsFindOpen[i]),tBin->findOpen(argsFindOpen[i])) << "Resposta errada ao calcular o find_open de i=" << argsFindOpen[i];
    } 
}

TEST_F(RMMTreeFixtureTest, bwdSearch_enclose){
    int k;
    for(int i=0;i<(t_v1->size)/2;i++){
        k = rand()%(t_v1->size);
        EXPECT_EQ(t_v1->enclose(k),tBin->enclose(k)) << "Resposta errada ao calcular o enclose de i=" << k;
    } 
}
